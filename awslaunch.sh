#!/bin/bash
echo usage awslanch.sh covid19pip/foldertoanalyze 64
...
sudo aws s3 sync s3://$1 ./

sudo aws s3 cp s3://covid19pip/make2files.pl ./
...
docker run -it -v /data:/data thom38/kaiju /kaiju/bin/kaiju -z $2 -t /data/kaijudb/nodes.dmp -f /data/kaijudb/kaiju_db.fmi -i /data/.R1.fastq.gz -j /data/.R2.fastq.gz -o /data/kaiju.out
...
sudo aws s3 cp kaiju.out.html s3://$1/
sudo aws s3 cp kaiju.out.krona s3://$1/
sudo aws s3 cp kaiju.out s3://$1/
